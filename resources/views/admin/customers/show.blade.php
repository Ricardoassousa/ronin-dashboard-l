...
<div class="max-w-7xl mx-auto p-6">
    ...
    <!-- Customer Info Card -->
    <div class="bg-white shadow rounded p-6 mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            <form action="{{ route('admin.customers.block', $customer) }}"
                ...
                <button type="submit"
                        class="{{ $customer->is_blocked ? 'bg-green-600 hover:bg-green-700' : 'bg-red-600 hover:bg-red-700' }}
                               text-white px-4 py-2 rounded shadow transition w-full sm:w-auto">
                ...
                <tbody>
                    ...
                    <tr class="border-t hover:bg-gray-50">
                        ...
                        <td class="p-3">
                            @switch($order->status)
                                @case('pending')
                                    <span class="px-2 py-1 text-sm bg-yellow-100 text-yellow-700 rounded">Pending</span>
                                    @break
                                @case('paid')
                                    <span class="px-2 py-1 text-sm bg-blue-100 text-blue-700 rounded">Paid</span>
                                    @break
                                @case('shipped')
                                    <span class="px-2 py-1 text-sm bg-green-100 text-green-700 rounded">Shipped</span>
                                    @break
                                @case('cancelled')
                                    <span class="px-2 py-1 text-sm bg-red-100 text-red-700 rounded">Cancelled</span>
                                    @break
                            @endswitch
                        </td>
